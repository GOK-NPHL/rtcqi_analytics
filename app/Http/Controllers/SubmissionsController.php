...
class SubmissionsController extends Controller
{
    ...
    public function getSPIData(Request $request)
    {
        ...
        try {
            ...
            $recordsReadData = [];
            $payload = [];
            if (isset($siteType) && !empty($siteType)) {
                for ($x = 0; $x < count($siteType); $x++) {
            ...
            // Ensure plain array (getFormRecords may return a League\Csv iterator)
            if (!is_array($rawResult)) {
                $rawResult = iterator_to_array($rawResult, true);
            }

            // Annotate each record with computed time-based followup stage
            $rawResult = $odkObj0->computeTimelineStages($rawResult);

            $page = $request->page ?? 1;
            $perPage = $request->perPage ?? 100;
            $total = count($rawResult);
            $pages = ceil($total / $perPage);
            $result_page = array_slice($rawResult, ($page - 1) * $perPage, $perPage);
            $result_s = [
                'headers' => $columnsToUse,
                'result' => $result_page ?? [],
                'total' => $total,
    ...
    private function getDataLoopOrgs(
        ...
        for ($x = 0; $x < count($orgUnitIds); $x++) {
            try {
                ...
                $orgUnitKey = $orgUnit['org_unit_id'] ?? null;
                if (!is_string($orgUnitKey) && !is_int($orgUnitKey)) {
                    Log::warning('SubmissionsController: skipping org unit with null/invalid org_unit_id');
                    continue;
                }

                if (array_key_exists($orgUnitKey, $recordsReadData)) {
                    $records = $recordsReadData[$orgUnitKey];
                } else {
                    $records = $this->getFormRecords($orgUnit);
                    $recordsReadData[$orgUnitKey] = $records;
                }
