...
function SPISubmissions() {
    ...
    const fetchOdkDataServer = useCallback(async (orgIds, timeline, stType, partners, sd, ed, pg = 1, ppg = 100) => {
        if (!orgIds || orgIds.length === 0) return;
        const returnedData = await FetchSPISubmissions(orgIds, timeline, stType, partners, sd, ed, pg, ppg);
        setLoading(false);
    ...
    useEffect(() => {
        (async () => {
            const returnedData = await FetchOrgunits();
            const defaultOrg = [returnedData.payload[0][0]['org_unit_id']] || [0];
            setOrgUnits(returnedData.payload[0]);
            setOrgUnitDataIds([defaultOrg[0]]);
            setCounties(returnedData.payload[0].filter(o => o.level === 2));
            fetchOdkDataServer(defaultOrg, [], siteType, [], '', '', 1, 100);
            FetchPartners().then(setAllPartners);
        })();
    }, []);

    const onFilter = () => fetchOdkDataServer(orgUnitDataIds, orgUnitTimeline, siteType, [], startDate, endDate, page, perPage);

    const handlePageChange = (pg) => {
        setPage(pg);
        fetchOdkDataServer(orgUnitDataIds, orgUnitTimeline, siteType, [], startDate, endDate, pg, perPage);
    };
                            ...
                            <div className="modal-content">
                                ...
                                <div className="modal-footer">
                                    ...
                                    <button type="button" disabled={isDownloading} className="btn btn-success" onClick={async () => {
                                        ...
                                        try {
                                            const result = await FetchSPISubmissions([countyDl], [], '', '', 1, 5000);
                ...
                <div className="col-md-12">
                    ...
                    <div className="table-responsive">
                        <table className="table table-striped table-condensed">
                            ...
                            <tbody>
                                {odkData?.length > 0 ? odkData.map((dt, indx) => (
                                    <tr key={indx + '_'}>
                                        {headers.slice(0, 8).map((header, i) => (
                                    ...
                                    <tr>
                                        <td colSpan={headers.length} style={{ textAlign: 'center' }}>
                                            {loading
                                                ? <div className="text-center"><i className="fa fa-spin fa-refresh"></i> Loading...</div>
...
const el = document.getElementById('SPISubmissions');
if (el) ReactDOM.createRoot(el).render(<SPISubmissions />);
